...
class CitySelectorViewController: UIViewController {
    ...
    lazy var searchVC: UISearchController = {
       let searchVc = UISearchController(searchResultsController: self.searchResultVC)
        ...
        searchVc.searchBar.backgroundColor = UIColor.clear
...
extension CitySelectorViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
         if indexPath.section == 0 {
            ...
            cell.onBtnClicked = {
                [weak self] city in
                self?.delegate?.didSelected(citySelector: self, city: city)
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        delegate?.didSelected(citySelector: self, city: city)
        print("点击了 \(String(describing: city))")
    }
...
extension CitySelectorViewController {
    fileprivate func getSearchResultArray(searchBarText: String) {
        ...
        searchResultVC.callBack = { [weak self] city in
            // 搜索完成 关闭resultVC
        ...
        if searchBarText.isIncludeChineseIn() {
            ...
            if searchBarText.count == 1 {
                ...
                for str in dic {
                    // 去空格
                    let py = String(str.chineseToPinyin().replacingOccurrences(of: " ", with: ""))
                    let range = py.range(of: searchBarText.lowercased())
                    if range != nil {
                        resultArray.append(str)
                    }
                }
